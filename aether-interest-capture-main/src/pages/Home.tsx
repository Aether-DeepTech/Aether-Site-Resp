...
export default function Home() {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-white font-creato">
      <header className="fixed top-0 left-0 w-full bg-white z-20">
        <nav className="container mx-auto flex items-center justify-between p-4">
          ...
          <Link to="/fale-com-especialista" className="border-2 border-black bg-transparent text-black font-bold uppercase text-sm md:text-base px-4 py-2 md:px-6 md:py-3 rounded-md transition-all duration-300 hover:bg-black hover:text-white">FALE COM UM ESPECIALISTA</Link>
        </nav>
      </header>
      ...
      <main className="flex-grow container mx-auto px-4 pt-40 md:pt-52 lg:pt-64">
        <div className="w-full max-w-5xl text-center mx-auto">
          <h1 className="text-3xl md:text-5xl font-bold uppercase mb-6 text-black">TRANSFORME O ATENDIMENTO DA SUA CLÍNICA COM INTELIGÊNCIA ARTIFICIAL</h1>
          <p className="uppercase text-gray-700 text-base md:text-xl leading-relaxed">MAIS AGENDAMENTOS, MENOS FALTAS E ATENDIMENTO 24H — TUDO PELO WHATSAPP COM NOSSO SISTEMA TREINADO ESPECIALMENTE PARA A ÁREA MÉDICA.</p>
        </div>
      </main>

      {/* SECÇÃO COM EFEITO STICKY E ALINHAMENTO CORRIGIDOS */}
          ...
          <div className="md:w-1/2">
            {features.map((feature, index) => (
              <div key={index} className="h-screen flex flex-col justify-center">
                <h2 className="text-3xl md:text-4xl font-bold uppercase mb-4">{feature.title}</h2>
                <p className="text-lg md:text-xl text-gray-600 uppercase">{feature.description}</p>
              </div>
            ))}
          </div>

          {/* COLUNA DA DIREITA: TELEMÓVEL QUE FICA FIXO E CENTRALIZADO */}
          <div className="md:w-1/2">
                ...
                <div className="max-w-sm mx-auto bg-white rounded-[2.5rem] border-4 border-gray-900 shadow-2xl">
                  <div className="w-full h-[600px] bg-gray-100 rounded-[2.2rem] flex flex-col overflow-hidden">
                    <div className="flex-shrink-0 flex items-center p-4 border-b-2 border-gray-300">
                      ...
                      <div className="ml-3 text-left"><p className="font-bold text-gray-800">AETHER IA</p><p className="text-sm text-green-600">ONLINE</p></div>
                    </div>
      ...
      <footer className="w-full bg-white py-8 mt-auto border-t border-gray-100">
        <div className="container mx-auto px-4 flex flex-col md:flex-row items-center justify-between space-y-4 md:space-y-0">
          ...
          <div className="text-center">
            <a href="mailto:contato@aetherdeeptech.com" className="text-gray-500 hover:text-black transition-colors uppercase text-sm tracking-wider">CONTATO@AETHERDEEPTECH.COM</a>
          </div>
          <div className="text-center">
            <p className="text-gray-500 uppercase text-xs tracking-wider">© 2025 AETHER INTELIGÊNCIA ARTIFICIAL. TODOS OS DIREITOS RESERVADOS.</p>
          </div>
        </div>
      </footer>
    </div>
